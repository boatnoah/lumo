...
type ReorderPayload = {
  ...
  order?: Array<{ promptId: number; position: number }>
}
...
export async function POST(request: Request) {
  ...
  if (!sessionId || !Array.isArray(order) || order.length === 0) {
    return NextResponse.json(
      { error: "Missing sessionId or slides" },
      { status: 400 }
    )
  }
  ...
  for (const { promptId, position } of order) {
    if (typeof promptId !== "number" || typeof position !== "number") {
      continue
    }

    const { error: updateError } = await supabase
      .from("prompts")
      .update({ slide_index: position })
      .eq("session_id", sessionIdFilter)
